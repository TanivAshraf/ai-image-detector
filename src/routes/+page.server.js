  ...
  analyzeImage: async ({ request }) => {
    ...
    try {
      ...
      if (!aiResponse.ok) {
         ...
         throw new Error(errorBody.error || "The AI model could not process the image.");
      }
...
async function saveToGithub(imageData, originalFilename, prediction) {
  ...
  const response = await fetch(GITHUB_API_URL, {
    ...
    body: JSON.stringify({
      ...
      content: content,
      // You could also add author info here if you want
    })
  });
  ...
  return responseData.content.html_url; // Return the URL to view the file on GitHub
}
